...
class AvalancheForecast(pydantic.BaseModel):
    ...
    # confidence: Optional[ForecastConfidences]
...
class ConfigObject(pydantic.BaseModel):
    ...
    round_destructive_up: bool

class RegionProperties(pydantic.BaseModel):
    centroid: list[float]
    id: str

class RegionGeometry(pydantic.BaseModel):
    type: str
    coordinates: list[list[list[tuple[float,float]]]]

class RegionFeature(pydantic.BaseModel):
    id: str
    type: str
    bbox: list[float]
    geometry: RegionGeometry

class RegionFeatureCollection(pydantic.BaseModel):
